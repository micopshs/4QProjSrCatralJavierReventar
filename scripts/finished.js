...
const make_histogram = function (attempt_arr) {
    ...
    for (const i of taken_attempt_arr.filter((attempt_number) => attempt_number > attempt_arr[0].wordLength + 1)) {
        histogram.set(-1, histogram.get(i) + (histogram.get(-1) ? histogram.get(-1) : 0));
        histogram.delete(i);
    }
    return histogram;
};
...
var load_page_data = function () {
    ...
    let past_attempts = JSON.parse(localStorage.getItem("VERBATIM_LS_past_attempts"));
